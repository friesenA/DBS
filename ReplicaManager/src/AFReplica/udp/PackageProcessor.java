...
import AFReplica.remoteObj.CustomerBankObj;
import AFReplica.remoteObj.ManagerBankObj;
import AFReplica.server.Account;
...
public class PackageProcessor extends Thread {
	...
	public PackageProcessor(DatagramPacket packet, HashMap<Character, ArrayList<Account>> customerRecords, String branch, int portNum) {
		this.packet = packet;
	...
	public void run() {
		...
		try {
			...
			if (arguments.get(3).equals("deposit")){
				String message = "";
				if(arguments.size() == 6){
					CustomerBankObj c = new CustomerBankObj(customerRecords, this.branch, portNum);
					double amount = (new Double(arguments.get(5)).doubleValue());
					message = c.deposit(arguments.get(4), amount);
				}
				else if (arguments.size() == 7){
					ManagerBankObj m = new ManagerBankObj(customerRecords, this.branch, portNum);
